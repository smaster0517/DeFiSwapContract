...
contract BNBRewardPhoenix is Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => uint256) public referralAmountLv1;
    mapping(address => uint256) public referralAmountLv2;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        safeTransferBNB(devaddr, tokenReward.div(10));
            ...
            if(pending > 0) {
                ...
                if(currentRewardBalance > 0) {
                    if(pending > currentRewardBalance) {
                        uint256 _withdrawFee = currentRewardBalance.mul(withdrawFee).div(100);
                        if (_withdrawFee>0)
                        {
                            currentRewardBalance = currentRewardBalance.sub(_withdrawFee);
                            safeTransferBNB(withdrawFeeAddress, _withdrawFee);
                        }
                        uint256 referAmountLv1 = currentRewardBalance.mul(percentForReferLv1).div(100);
                        uint256 referAmountLv2 = currentRewardBalance.mul(percentForReferLv2).div(100);
                        _transferReferral(referAmountLv1, referAmountLv2);

                        currentRewardBalance = currentRewardBalance.sub(referAmountLv1);
    ...
    function _transferReferral(uint256 _referAmountLv1, uint256 _referAmountLv2) internal {
        ...
        if (referrerLv1 != address(0)) {
            uint256 lpStaked = payable(address(referrerLv1)).balance;
            if (lpStaked >= stakeAmountLv1 && _referAmountLv1>0 ) {
                safeTransferBNB(referrerLv1, _referAmountLv1);
                referralAmountLv1[address(referrerLv1)] = referralAmountLv1[address(referrerLv1)].add(_referAmountLv1);
            } else {
                referAmountForDev = referAmountForDev.add(_referAmountLv1);
            }
        
            address referrerLv2 = userRef[referrerLv1];
            uint256 lpStaked2 = payable(address(referrerLv2)).balance;
            if (referrerLv2 != address(0) && lpStaked2 >= stakeAmountLv2   && _referAmountLv2>0 ) {
             
                safeTransferBNB(referrerLv2, _referAmountLv2);
                referralAmountLv2[address(referrerLv2)] = referralAmountLv2[address(referrerLv2)].add(_referAmountLv2);
        ...
        if (referAmountForDev > 0) {
            safeTransferBNB(devaddr, referAmountForDev);
        }
    }

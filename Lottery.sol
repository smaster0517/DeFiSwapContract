...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // State variables 
    // Instance of Pnixs token (collateral currency for lotto)
    IERC20 internal pnixs_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInPnixs;    // The amount of pnixs for prize money
        uint256 costPerTicket;      // Cost per ticket in $pnixs
     ...
     * @param   _prizePoolInPnixs The amount of Pnixs available to win in this 
     *          lottery.
        ...
        returns(uint256 lotteryId)
    {
        ...
        require(
            _prizePoolInPnixs != 0 && _costPerTicket != 0,
    ...
    function withdrawPnixs(uint256 _amount) external onlyOwner() {
        pnixs_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        // Transfers the required pnixs to this contract
        pnixs_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInPnixs = allLotteries_[_lotteryId].prizePoolInPnixs.sub(prizeAmount);
        // Transfering the user their winnings
        pnixs_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        pnixs_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in pnixs the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInPnixs.mul(perOfPool);
